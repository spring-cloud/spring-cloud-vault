...
By integrating Vault with Spring Boot, you can dynamically retrieve and manage X509 certificates for your applications.
...
[[vault.ssl-bundles.configuration]]
== Vault-managed SSL Bundle Configuration

Vault-managed certificates can be configured either through properties or by registering a javadoc:org.springframework.cloud.vault.ssl.VaultSslBundleRegistrar[].

When used to application properties, an SSL bundle to secure an embedded web server is typically configured as shown in this example:
...
NOTE: Vault-managed SSL bundles generate a random pass phrase for the underlying `KeyStore` each time SSL bundles are created or updated.
Since applications do not need to access the `KeyStore` directly, the pass phrase is less significant than in a manual configuration approach.

[[vault.ssl-bundles.configuration.properties]]
== Property-based SSL Bundle Configuration
...
== Property-based Truststore (Issuer CA) SSL Bundle Configuration

When using Vault-managed SSL bundles, you might also want to configure a truststore that contains the issuer CA certificates to validate peer certificates to solely rely on Vault as the primary certificate authority.
Spring Cloud Vault supports configuring such truststores using Vault-managed SSL bundles as well.
The configuration properties are similar to the previously described `spring.cloud.vault.ssl.bundle` properties but use the `spring.cloud.vault.ssl.issuer-bundle` prefix with a slightly different set of options.

A complete configuration example of the `spring.cloud.vault.ssl.issuer-bundle` properties is as follows:
...
[[vault.ssl-bundles.configuration.api]]
