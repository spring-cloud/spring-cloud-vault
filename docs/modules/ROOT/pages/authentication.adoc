...
[[vault.config.authentication.github]]
== GitHub authentication

You can authenticate against Vault using GitHub authentication by providing a personal access token.
As a fallback, the token may also be retrieved the https://cli.github.com/[GitHub CLI] if installed and authenticated.
...
* `authentication` setting this value to `GITHUB` selects the GitHub authentication method
* `token` sets the static token to use. If missing or empty, then an attempt can be made to retrieve a token from `gh auth token` if the GitHub CLI is installed and authenticated and if the access is allowed through `spring.cloud.vault.github.allow-cli-token=true`.
* `allow-cli-token` enables the fallback to the GitHub CLI to retrieve a token. Disabled by default to not invoke the GitHub CLI unexpectedly.
