...
.Customizing `VaultClient` or `ReactiveVaultClient`
[source,java]
----
InstanceSupplier<VaultClient> supplier = context -> {
...
SpringApplication application = new SpringApplication(MyApplication.class);
application.addBootstrapRegistryInitializer(registry -> registry.register(VaultClient.class, supplier));
----
