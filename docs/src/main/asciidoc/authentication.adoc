...
As a fallback, the token may also be retrieved from `spring.cloud.vault.token-file` or `~/.vault-token` which is the default location used by the Vault CLI to cache tokens.
...
* `token` sets the static token to use. If missing or empty, then an attempt will be made to retrieve a token from `spring.cloud.vault.token-file` or ~/.vault-token.
