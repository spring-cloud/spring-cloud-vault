...
/**
 * Unit tests for {@link CertificateRotationContainer}.
 *
 * @author Mark Paluch
 */
class CertificateRotationContainerUnitTests {
	...
	void shouldConsiderJitter() {
		...
		Duration renewal1 = CertificateRotationContainer.getRenewalDelay(clock, expiry, threshold);
		Duration renewal2 = CertificateRotationContainer.getRenewalDelay(clock, expiry, threshold);
	...
	void shouldConsiderThresholdTime() {
		...
		Duration renewalSeconds = CertificateRotationContainer.getRenewalDelay(clock, expiry, threshold);
	...
	void shouldReportInstantRenewalTime() {
		...
		Duration renewal = CertificateRotationContainer.getRenewalDelay(clock, expiry, threshold);
