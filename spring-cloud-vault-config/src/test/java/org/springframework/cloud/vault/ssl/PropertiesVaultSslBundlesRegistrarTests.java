...
class PropertiesVaultSslBundlesRegistrarTests {
	...
	void shouldRegisterSimpleBundle() {
		...
		runner.withPropertyValues("spring.cloud.vault.ssl.bundle.localhost.role-name=my-role").run(ctx -> {
			DefaultVaultSslBundleRegistry registry = new DefaultVaultSslBundleRegistry();
			PropertiesVaultSslBundlesRegistrar registrar = ctx.getBean(PropertiesVaultSslBundlesRegistrar.class);
			registrar.register(registry);
			...
			assertThat(request.name()).isEqualTo("localhost");
			assertThat(request.roleName()).isEqualTo("my-role");
			assertThat(request.sslProtocol()).isEqualTo(SslBundle.DEFAULT_PROTOCOL);
			assertThat(request.certificateRequest().getCommonName()).isEqualTo("localhost");
			assertThat(request.certificateRequest().getTtl()).isNull();
		});
	}
	...
	void shouldRegisterFullBundle() {
		...
		runner
			...
			.run(ctx -> {
				...
				VaultCertificateRequest cert = request.certificateRequest();
