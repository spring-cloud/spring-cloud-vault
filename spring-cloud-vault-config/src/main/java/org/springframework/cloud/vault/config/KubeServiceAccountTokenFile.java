 ...
 * @see KubeJwtSupplier
 */
class KubeServiceAccountTokenFile implements KubeJwtSupplier {
	...
	KubeServiceAccountTokenFile() {
		this(DEFAULT_KUBERNETES_SERVICE_ACCOUNT_TOKEN_FILE);
	}

	KubeServiceAccountTokenFile(String fileName) {
		this(new FileSystemResource(fileName));
	}

	KubeServiceAccountTokenFile(File file) {
		this(new FileSystemResource(file));
	}

	KubeServiceAccountTokenFile(Resource resource) {
